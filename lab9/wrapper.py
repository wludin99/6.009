...
def read_map(path):
    ...
    with open(path, 'r') as map_file:
        ...
        game_info['rocks'] = set(map(tuple, game_info['rocks'])).copy()

        # Set defaults.
        game_info['money'] = game_info.get('money', 15)
        game_info['num_allowed_unfed'] = game_info.get('num_allowed_unfed', 10)
        game_info['spawn_interval'] = game_info.get('spawn_interval', 25)
        game_info['animal_speed'] = game_info.get('animal_speed', 3)
