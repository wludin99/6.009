...
function display(data) {
    ...
    formations.forEach(function(formation) {
        ...
        if (svg === undefined) {
            ...
            g += '>';
            g += svg;
            // unused variables -- replace x and y with these 5 lines below for wireframe coords to be the centers (not corner)
            // var x_cen = (formation.rect[0]-(formation.rect[2]-1)/2).toString();
            // var y_cen = (formation.rect[1]-(formation.rect[3]-1)/2).toString();
            if (debug) {
...
function init_gui() {
    ...
    invoke_rpc("/ls", {"path":"cases/"}, 0, function(loaded) {
        ...
        // // if a valid one is stored, use it
